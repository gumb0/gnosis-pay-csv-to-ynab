...
description: Convert Gnosis Pay CSV exports into YNAB-compatible CSV files. Use when working with transaction files that include columns like `date`, `merchant_name`, `billing_amount`, `status`, and `kind`, and you need YNAB columns `Date,Payee,Memo,Amount` with date normalized to `YYYY-MM-DD` and amount sign derived from transaction status.
---
...
- `Amount`: take absolute value of `billing_amount`, then:
  ...
  - if `status` is `Reversal` and `kind` is `Payment`, output as negative
  - if `status` is `Reversal` and `kind` is `Reversal`, output as positive
...
- Fail fast if required columns are missing (`date`, `merchant_name`, `billing_amount`, `status`, `kind`).
